...
// Contrast-safe palette (all pass 4.5:1 on #f7f5f2 or #ffffff)
// #1a1a1a on #f7f5f2 = ~16:1 ✓
// #4a4a4a on #f7f5f2 = ~7.5:1 ✓
// #595959 on #ffffff = ~7:1 ✓
// #166534 (dark green) on #ffffff = ~7:1 ✓  (for "X left")
// #9a3412 (dark orange) on #ffffff = ~5.5:1 ✓ (for "X over")
// #c2410c on #fff7ed = ~5:1 ✓ (tip label on light orange bg)
// #1e3a5f on #eff6ff = ~8:1 ✓ (button)
...
export default function WellnessTracker() {
  ...
  const [meals, setMeals] = useState(MEAL_SLOTS.map(m => ({ ...m, cal: 0, breakdown: null, description: "" })));
  ...
  // Bar fill colors — decorative only, not text
  ...
  async function logMeal(i) {
    ...
    try {
      ...
      alert("Couldn't calculate calories. Please try again.");
    }
    const loading2 = [...mealLoading];
    loading2[i] = false;
    setMealLoading(loading2);
  }

  function editMeal(i) {
    const inp = [...mealInputs];
    inp[i] = meals[i].description;
    setMealInputs(inp);
    const ed = [...editing];
    ed[i] = true;
    setEditing(ed);
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {isLogged && (
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => editMeal(i)} style={{ background: "#f0ede8", border: "1px solid #d6d3ce", borderRadius: 8, padding: "4px 10px", color: "#1a1a1a", cursor: "pointer", fontSize: 12, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>Edit</button>
                        <button onClick={() => clearMeal(i)} style={{ background: "#fff1ee", border: "1px solid #fecdbb", borderRadius: 8, padding: "4px 10px", color: "#9a3412", cursor: "pointer", fontSize: 12, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>Clear</button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {isLogged ? (
                  <div style={{ background: "#f7f5f2", borderRadius: 10, padding: "12px 14px" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                      {[
                        { label: "cal", val: meal.breakdown.total },
                        { label: "protein", val: `${meal.breakdown.protein}g` },
                        { label: "carbs", val: `${meal.breakdown.carbs}g` },
                        { label: "fat", val: `${meal.breakdown.fat}g` },
                      ].map(s => (
                  ...
                  <div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input
                        ...
                        style={{ flex: 1, background: "#f7f5f2", border: "1px solid #c8c4be", borderRadius: 10, padding: "9px 14px", color: "#1a1a1a", fontSize: 14, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}
                      />
                      <button onClick={() => logMeal(i)} disabled={mealLoading[i]} style={{
                        background: mealLoading[i] ? "#e5e2dd" : "#1a1a1a",
                        border: "none", borderRadius: 10,
                        padding: "9px 14px",
                        color: mealLoading[i] ? "#4a4a4a" : "#ffffff",
                        cursor: mealLoading[i] ? "default" : "pointer",
                    ...
                    {editing[i] && (
                      <button onClick={() => { const ed = [...editing]; ed[i] = false; setEditing(ed); }} style={{ marginTop: 6, background: "none", border: "none", color: "#4a4a4a", fontSize: 12, cursor: "pointer", fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", padding: 0, textDecoration: "underline" }}>
                        ← cancel
                      </button>
                    )}
                    {mealLoading[i] && (
                      <div style={{ marginTop: 8, fontSize: 12, color: "#4a4a4a" }}>✨ Analyzing your meal...</div>
                    )}
                  </div>
                )}
              </div>
